<script lang="ts">
	...
	onMount(() => {
		...
		const resizeObserver = new ResizeObserver((entries) => {
			...
			// Debounce so a window resize doesn't rebuild the simulation per frame
			clearTimeout(resizeTimer);
			resizeTimer = setTimeout(() => (width = w), 100);
		});
	...
	// Re-render when the container width or any graph prop changes
	$effect(() => {
		if (width > 0) {
			render();
		}
	...
	function render() {
		...
		simulation?.stop();
		select(container).selectAll('svg, .graph-tooltip').remove();

		const { nodes, links } = buildGraph();
		if (nodes.length === 0) return;

		const maxConnections = max(nodes, (d) => d.connections) ?? 1;
		const radiusScale = scaleSqrt().domain([0, maxConnections]).range([4, 18]);
		...
		const sim = forceSimulation(nodes)
			...
			.force(
				'collision',
				forceCollide<GraphNode>().radius((d) => radiusScale(d.connections) + 2)
			);
		simulation = sim;
		...
		node
			...
			.attr('stroke', (d) => (d.id === currentSlug ? 'var(--cds-focus, #0f62fe)' : 'none'))
		...
		node
			...
			.on('mouseout', () => {
				...
				node.select('circle').attr('opacity', (d) => (d.id === currentSlug ? 1 : 0.6));
